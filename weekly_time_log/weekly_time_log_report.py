...
difference_dict = dict(zip(x, difference_y_net))
difference_dict = {x:y for x, y in difference_dict.items() if abs(y) >=2 }

negative_difference_dict = {x:y for x, y in difference_dict.items() if abs(y) >=2 and y < 0 }
positive_difference_dict = {x:y for x, y in difference_dict.items() if abs(y) >=2 and y > 0 }


final_x = list(difference_dict.values())
final_y = list(difference_dict.keys())
negative_final_x = list(negative_difference_dict.values())
negative_final_y = list(negative_difference_dict.keys())
positive_final_x = list(positive_difference_dict.values())
positive_final_y = list(positive_difference_dict.keys())
...
fig.append_trace(go.Bar(
    ...
    name='SenseAuto v1.2.0 同比 v1.1.0 耗时减少 (ms)',
    orientation='h',
), 1, 1)
...
fig.append_trace(go.Bar(
    ...
    y= positive_final_y,
    marker=dict(
        color='rgba(100, 106, 88, 0.7)',
        line=dict(
            color='rgba(74, 89, 61, 0.6)',
            width=1),
    ),
    name='SenseAuto v1.2.0 同比 v1.1.0 耗时增长 (ms)',
    orientation='h',
), 1, 1)
...
fig.update_layout(
    title="<b>{} 各模块函数耗时平均 (ms)</b><br>'{}',同比v1.1.0增加耗时<b>{:.1f}ms</b><br>'{}',同比v1.1.0减少耗时<b>{:.1f}ms</b>"\
    .format("SenseAuto v1.2.0",positive_rising_module,max(difference_y_net),negative_rising_module, min(difference_y_net)),
    yaxis=dict(
        showgrid=False,
        showline=False,
        showticklabels=True,
...
y_s = np.round(final_x, decimals=2)


# Adding labels
for yd, xd in zip(y_s, final_y):
