...
if __name__ == "__main__":
    ...
    visible_list = len(subplot_titles_list) * [False]
    button_dict = []

    for key in subplot_titles_list:
        x = np.arange(len(Entire_Dict[key][0::interval]))
        visible_list[subplot_titles_list.index(key)] = True
        ...
        fig.add_trace(go.Scatter(x=x, 
                                 y=signal.savgol_filter(Entire_Dict[key][0::interval], 91, 5), # order of fitted polynomial # window size used for filtering 
                                 mode='lines',
                                 name= '{} (ms)'.format(key),
        ...
        button_dict.append(dict(label="{}".format(key),method="update",args=[{"visible": visible_list},{"title": "{}".format(key), "annotations": []}]))
        visible_list = len(subplot_titles_list) * [False]
